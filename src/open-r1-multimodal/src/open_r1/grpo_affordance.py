...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if 'image_path' in example:
            ...
            w, h = image.size
            max_dim = max(w, h)
            new_image = Image.new("RGB", (max_dim, max_dim), (122, 117, 104))
...
def trajectory_reward_interp(prompts, completions, solution, **kwargs):
    import re
    pattern = r"\[(.*?)\]"
    rewards = []
...
def trajectory_reward(prompts, completions, solution, **kwargs):
    import re
    pattern = r"\[(.*?)\]"
    rewards = []
    for pred, sol in zip(completions, solution):
        ...
        try:
            ...
            sol_pts = sol
            # 简单MSE奖励
            mse = sum((px - sx)**2 + (py - sy)**2 for (px, py), (sx, sy) in zip(pred_pts, sol_pts)) / len(pred_pts)
            rewards.append(max(0.0, 1.0 - mse / 10000))
        except:
            rewards.append(0.0)
    return rewards

def trajectory_smooth_reward(prompts, completions, solution, **kwargs):
    import re
    pattern = r"\[(.*?)\]"
    rewards = []
    for pred in completions:
        ...
        try:
            ...
            rewards.append(max(0.0, 1.0 - smoothness / 1000))
        except:
            rewards.append(0.0)
    return rewards
...
def main(script_args, training_args, model_args):
    ...
    # Load the dataset
    dataset = LazySupervisedDataset(script_args.dataset_name, script_args, question_template=vlm_module_cls.get_question_template(task_type="affordance_long"))
    # Check the shape of the first dataset, set a breakpoint
    # from pprint import pprint
    # print("Loaded", len(dataset), "samples")
    # print("First entry keys:", dataset[0].keys())
    # img = dataset[0]["image"]
    # print("Image size (W, H):", img.size)

    # print("Instruction:", pprint(dataset[0]["prompt"]))
